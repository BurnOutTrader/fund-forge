...
impl HybridStorage {
    ...
    /// Used to retro-actively compress data that was downloaded before compression was added, will be removed from future versions
    pub async fn compress_historical_data(&self, batch_len: usize) -> io::Result<()> {
        let base_path = &self.base_path;

        // Iterate through data vendor directories
        for vendor_entry in fs::read_dir(base_path)? {
            let vendor_dir = vendor_entry?.path();
            if !vendor_dir.is_dir() { continue; }

            // Iterate through market type directories
            for market_entry in fs::read_dir(&vendor_dir)? {
                let market_dir = market_entry?.path();
                if !market_dir.is_dir() { continue; }

                // Accumulate symbols in a batch of 10
                let mut batch = Vec::new();
                for symbol_entry in fs::read_dir(&market_dir)? {
                    let symbol_dir = symbol_entry?.path();
                    if !symbol_dir.is_dir() { continue; }

                    batch.push(symbol_dir);

                    // If we reach a batch size of 10, process the batch
                    if batch.len() == batch_len {
                        HybridStorage::process_batch(self, &batch).await?;
                        batch.clear(); // Clear the batch to start a new one
                    }
                }

                // Process any remaining symbols in the batch
                if !batch.is_empty() {
                    HybridStorage::process_batch(self, &batch).await?;
                }
            }
    ...
    async fn process_batch(this: &Self, batch: &[PathBuf]) -> io::Result<()> {
        // Compress all symbols in the batch concurrently
        let compressions = batch.iter().map(|symbol_dir| {
            println!("Compressing data for symbol: {}", symbol_dir.file_name().unwrap().to_string_lossy());
            this.compress_symbol_directory(symbol_dir)
        });

        // Run all compressions concurrently and handle errors
        join_all(compressions).await.into_iter().collect::<Result<(), io::Error>>()?;

        for symbol_dir in batch {
            println!("Finished compressing symbol: {}", symbol_dir.file_name().unwrap().to_string_lossy());
        }
    ...
    /// Used to retro actively compress data that was downloaded before compression was added, will be removed from future versions
    async fn compress_symbol_directory(&self, symbol_dir: &Path) -> io::Result<()> {
        let mut total_original_size = 0u64;
        let mut total_compressed_size = 0u64;
        let mut files_processed = 0;

        // Iterate through resolution directories
        for res_entry in fs::read_dir(symbol_dir)? {
            let res_dir = res_entry?.path();
            if !res_dir.is_dir() { continue; }

            // Iterate through data type directories
            for dtype_entry in fs::read_dir(&res_dir)? {
                let dtype_dir = dtype_entry?.path();
                if !dtype_dir.is_dir() { continue; }

                // Iterate through year directories
                for year_entry in fs::read_dir(&dtype_dir)? {
                    ...
                    for month_entry in fs::read_dir(&year_dir)? {
                        ...
                        // Process all .bin files in the month directory
                        for file_entry in fs::read_dir(&month_dir)? {
                            ...
                            // Read original file using old mmap method
                            let mmap = self.old_get_or_create_mmap(&file_path).await?;
                            let data = BaseDataEnum::from_array_bytes(&mmap.to_vec())
                            ...
                            let bytes = BaseDataEnum::vec_to_bytes(data);
                            ...
                            if files_processed % 100 == 0 {
                                println!(
                                    "Processed {} files for symbol {}. Current reduction: {:.1}%",
                                    files_processed,
                                    symbol_dir.file_name().unwrap().to_string_lossy(),
                                    ((total_original_size as f64 - total_compressed_size as f64) / total_original_size as f64 * 100.0)
                                );
                            }
                        }
                    }
    ...
    async fn old_get_or_create_mmap(&self, file_path: &Path) -> io::Result<Arc<Mmap>> {
