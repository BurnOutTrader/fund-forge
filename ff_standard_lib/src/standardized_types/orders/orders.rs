...
pub async fn on_order_update_event(event: OrderUpdateEvent) {
    match event {
        OrderUpdateEvent::Accepted{id, mut order, symbol_name, time_utc, brokerage,account_id} => {
            order.id = id.clone();
            order.state = OrderState::Accepted;
            order.time_created_utc = time_utc;
            if !orders_pending.contains_key(&symbol_name) {
                orders_pending.insert(symbol_name, HashMap::new());
            }
            self.order_accepted(order).await;
        },
        OrderUpdateEvent::Filled{ id, symbol_name, quantity_filled, average_price, fees, time_utc, brokerage,account_id } =>{
            if let Some(inner_map) = orders_pending.get_mut(&symbol_name) {
                let mut order = match inner_map.remove(&id) {
                    Some(order) => order,
                    None => {
                        println!("Order with id {} not found in ledger", id);
                        return;
                    }
                };
                ...
                order.fees = fees;
                order.time_filled_utc = Some(time_utc);
                let mut orders_completed = self.orders_completed.write().await;
                if let Some(completed_orders) = orders_completed.get_mut(&symbol_name) {
                    completed_orders.insert(id, order);
                } else {
                    let mut new_inner_map = HashMap::new();
                    new_inner_map.insert(id, order);
                    orders_completed.insert(symbol_name.clone(), new_inner_map);
                }
            }
        }
        OrderUpdateEvent::PartiallyFilled{id,symbol_name, quantity_filled, average_price, fees, time_utc, brokerage,account_id} => {
            if let Some(inner_map) = orders_pending.get_mut(&symbol_name) {
                if let Some(order) = inner_map.get_mut(&id) {
                    order.average_fill_price = Some(average_price);
                    order.quantity_filled = quantity_filled;
        ...
        OrderUpdateEvent::Cancelled{id, symbol_name, brokerage,account_id} => {
            if let Some(inner_map) = orders_pending.get_mut(&symbol_name) {
                let mut order = match inner_map.remove(&id) {
                    Some(order) => order,
                    None => {
                ...
                self.complete_order(symbol_name, id).await;
            }
        },
        OrderUpdateEvent::Rejected{id, symbol_name, reason, brokerage,account_id} =>  {
            println!("Order Rejected: {}", reason);
        },
        OrderUpdateEvent::Updated{ id, symbol_name, change_type, brokerage,account_id } => {
            if let Some(inner_map) = orders_pending.get_mut(&symbol_name) {
                if let Some(order) = inner_map.get_mut(&id) {
                    match change_type {
                        OrderChangeType::LimitPrice(price) => order.limit_price = Some(price),
                        OrderChangeType::TriggerPrice(price) => order.trigger_price = Some(price),
                        OrderChangeType::TimeInForce(tif) => order.time_in_force = tif,
                        OrderChangeType::Quantity(q) => order.quantity_ordered = q,
                        OrderChangeType::Tag(tag) => order.tag = tag
                    }
                }
            }
        }
        OrderUpdateEvent::UpdateRejected{ id, symbol_name, change_type, reason , brokerage,account_id} => {
            println!("Order {} update rejected: {}", change_type, reason)
        }
    }
}
*/