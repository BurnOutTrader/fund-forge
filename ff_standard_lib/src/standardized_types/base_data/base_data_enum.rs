...
impl Display for BaseDataEnum {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            BaseDataEnum::Price(price) => write!(
                f,
                "{}: {}, {}: {}",
...
impl BaseDataEnum {
    ...
    pub fn base_data_type(&self) -> BaseDataType {
        match self {
            BaseDataEnum::Price(_) => BaseDataType::TradePrices,
    ...
    pub fn folder_path(
        ...
        let folder_structure = format!(
            "{}/{}/{}/{}/{}",
            &subscription.symbol.data_vendor.to_string(),
            &subscription.market_type.to_string(),
            &subscription.base_data_type.to_string(),
            &subscription.resolution.to_string(),
            &cleaned_symbol
        );
    ...
    pub fn save_as_rkyv(
        data_folder: &PathBuf,
        price_data_enum_vec: Vec<BaseDataEnum>,
        subscription: &DataSubscription,
    ) -> std::io::Result<()> {
    ...
    pub fn format_and_save(
        ...
        price_data_enum_vec: BTreeMap<DateTime<Utc>, BaseDataEnum>,
        subscription: &DataSubscription,
    ) -> Result<(), Box<dyn std::error::Error>> {
        let mut data_dict: BTreeMap<String, Vec<BaseDataEnum>> = BTreeMap::new();
        println!(
            "Formatting and saving {} data for: {}",
            price_data_enum_vec.len(),
    ...
    pub fn file_name_parse_date(string: &str) -> DateTime<Utc> {
        ...
        let datetime_part = parts
            .get(2)
            .expect("Unexpected filename format: missing datetime part");

        // Splitting the datetime_part to separate the date from the extension, assuming .rkyv extension
        let date_part = datetime_part
            .split(".")
            .next()
            .expect("Unexpected datetime format: missing date part");
    ...
    fn get_all_entries(data_folder: &PathBuf, subscription: &DataSubscription) -> Vec<PathBuf> {
        ...
        let entries: Vec<_> = fs::read_dir(data_folder)
            .unwrap()
            .filter_map(|res| {
                let path = res.ok()?.path();
                let file_name = path.file_name()?.to_str()?;
                if file_name.starts_with(&symbol) {
                    // Make sure to use &symbol to get a &str from String
                    Some(path)
                } else {
                    None
                }
    ...
    pub fn earliest_time_available(
        data_folder: &PathBuf,
        subscription: &DataSubscription,
    ) -> Option<DateTime<Utc>> {
    ...
    pub fn latest_time_available(
        ...
        let load_data = match BaseDataEnum::from_array_bytes(&file) {
            Ok(data) => data,
            Err(_e) => return Some(Self::file_name_parse_date(&file_name)),
        };
    ...
    pub fn earliest_data_available(
        data_folder: &PathBuf,
        subscription: &DataSubscription,
    ) -> Option<Vec<BaseDataEnum>> {
        let mut entries: Vec<_> = Self::get_all_entries(data_folder, &subscription);
        ...
        entries.sort_by_key(|entry| {
            Self::file_name_parse_date(
                &entry
                    .file_name()
                    .expect("Failed to parse data and sort")
                    .to_string_lossy()
                    .into_owned(),
            )
        });
    ...
    pub fn latest_available_data(
        ...
        entries.sort_by_key(|entry| {
            Self::file_name_parse_date(
                &entry
                    .file_name()
                    .expect("Failed to parse date and sort")
                    .to_string_lossy()
                    .into_owned(),
            )
        });
