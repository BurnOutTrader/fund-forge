...
pub enum ConnectionType {
    Vendor(DataVendor), //todo remove data vendor and brokerage, only support a single data server to keep thiings simple when getting historical data or improve get_requests data fn
    Broker(Brokerage),
    Default,
    StrategyRegistry,
}
