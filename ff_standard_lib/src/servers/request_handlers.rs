...
use crate::standardized_types::data_server_messaging::{
    AsyncRequestType, BaseDataPayload, FundForgeError, SynchronousRequestType,
    SynchronousResponseType,
};
    ...
    tokio::task::spawn(async move {
        while let Some(data) = communicator.receive(true).await {
            ...
            let response = match request {
                ...
                SynchronousRequestType::SymbolsBroker(broker, market) => {
                    broker.symbols_response(market).await
                }
                SynchronousRequestType::Resolutions(vendor, market_type) => {
            ...
            let response =
                response.unwrap_or_else(|ff_error| SynchronousResponseType::Error(ff_error.into()));
...
pub(crate) async fn base_data_response(
