...
impl Brokerage {
    pub async fn margin_required(&self, _symbol_name: &Symbol, quantity: u64) -> f64 {
        match self {
            Brokerage::Test => {
                quantity as f64 * 2500.0
            }
        }
    }

    pub async fn paper_ledger(&self, account_id: AccountId, currency: AccountCurrency, cash_value: f64) -> Ledger {
        match self {
            Brokerage::Test => {
...
impl FromStr for Brokerage {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            _ => Err(FundForgeError::ClientSideErrorDebug(format!("Invalid brokerage string: {}", s))),
        }
    }
}
...
pub mod client_requests {
    ...
    pub trait ClientSideBrokerage: Sync + Send {
        ...
        async fn account_currency(&self, account_id: AccountId) -> Result<AccountCurrency, FundForgeError>;
    ...
    impl Brokerage {
        ...
        pub async fn synchronous_client(&self) -> Arc<SynchronousCommunicator> {
            get_synchronous_communicator(ConnectionType::Broker(self.clone())).await
        }

 /*       pub async fn async_receiver(&self) -> Arc<Mutex<SecondaryDataReceiver>> {
            get_readside_client(ConnectionType::Broker(self.clone())).await
        }

        pub async fn async_sender(&self) -> Arc<Mutex<SecondaryDataSender>> {
            get_writeside_client(ConnectionType::Broker(self.clone())).await
        }*/
    }
}