                ...
                match request {
                    GuiRequest::ListAllStrategies => {
                        ...
                        match strategies.is_empty() {
                            true => {},
                            false => match sender.send(&RegistryGuiResponse::ListStrategiesResponse(strategies).to_bytes()).await {
                                Ok(_) => {}
                                Err(_) => {}
                            }
                        }
                    ...
                    GuiRequest::Subscribe(owner) => {
                        ...
                        match sender.send(&RegistryGuiResponse::Subscribed(owner.clone(), buffer).to_bytes()).await {
                            Ok(_) => {}
                            Err(_) => {}
                        }
                    }
