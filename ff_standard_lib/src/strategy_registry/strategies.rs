...
impl Bytes<Self> for StrategyResponse {
    ...
    fn from_bytes(archived: &[u8]) -> Result<StrategyResponse, FundForgeError> {
        ...
        match rkyv::from_bytes::<StrategyResponse>(archived) { //Ignore this warning: Trait `Deserialize<UiStreamResponse, SharedDeserializeMap>` is not implemented for `ArchivedUiStreamResponse` [E0277] 
...
impl Bytes<Self> for StrategyRequest {
    ...
    fn from_bytes(archived: &[u8]) -> Result<StrategyRequest, FundForgeError> {
        // If the archived bytes do not end with the delimiter, proceed as before
        match rkyv::from_bytes::<StrategyRequest>(archived) { //Ignore this warning: Trait `Deserialize<UiStreamResponse, SharedDeserializeMap>` is not implemented for `ArchivedUiStreamResponse` [E0277] 
            Ok(response) => Ok(response),
            Err(e) => {
                Err(FundForgeError::ClientSideErrorDebug(e.to_string()))
            }
