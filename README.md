...
- 12-11-24: Indicators are no longer an enum, we now pass a Box<Self> (indicator::new() should return Box<self>) to the subscribe_indicator function, this allows us to use the same indicator in multiple strategies without cloning the indicator. [see](ff_standard_lib/src/strategies/STRATEGIES_README.md/#creating-indicators)
